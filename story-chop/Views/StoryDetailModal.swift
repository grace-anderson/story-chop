...
struct StoryDetailModal: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showSharingSheet) {
            SharingSheet(activityItems: shareItems) {
                // Clean up shared file after sharing
                if let fileURL = shareItems.first as? URL {
                    sharingService.cleanupSharedFile(at: fileURL)
                }
            }
        }
    }
    ...
    private func shareStory() {
        ...
        case .success(let items):
            print("[DEBUG] Share items prepared successfully, showing sharing sheet")
            shareItems = items
